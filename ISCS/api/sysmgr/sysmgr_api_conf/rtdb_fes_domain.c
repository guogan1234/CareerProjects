...
/*
 * 功能：组合实时库FES服务器列表信息和域服务器列表IP信息
 * 说明：组合实时库FES服务器列表信息和域服务器列表IP信息
 * 参数：datas[in] - 域服务器列表IP信息
 *      list[in] - 实时库FES服务器列表信息
 * 返回值：实时库FES服务器列表配置信息
 */
sys_conf_rtdb_fes_server* sys_conf_match_combine_rtdb_fes_server(sys_conf_domain_server* datas,sys_conf_rtdb_fes_server_list* list){
    if(datas == NULL || list == NULL){
        return NULL;
    }
    sys_conf_domain_server* temp = NULL;
    temp = datas;
    sys_conf_rtdb_fes_server* temp_servers = NULL;
 ...
 * 返回值：实时库FES服务器列表信息
 */
sys_conf_rtdb_fes_server_list* sys_conf_get_rtdb_fes_server(mxml_node_t* node,mxml_node_t* xml){
    if(node == NULL || xml == NULL){
        return NULL;
    }

    sys_conf_rtdb_fes_server_list* ret = (sys_conf_rtdb_fes_server_list*)malloc(sizeof(sys_conf_rtdb_fes_server_list));

    sys_conf_rtdb_fes_server* prev = NULL;
    sys_conf_rtdb_fes_server* first = NULL;
    int size = 0;
    mxml_node_t* tnode = mxmlFindElement(node,xml,"SERVER","server_id",NULL,MXML_DESCEND);
    do{
        if(tnode->type == MXML_ELEMENT){
        sys_conf_rtdb_fes_server* servers = (sys_conf_rtdb_fes_server*)malloc(sizeof(sys_conf_rtdb_fes_server));//not free
        servers->next = NULL;
        if(first ==NULL){
            first = servers;
        }
        ...
        mxml_node_t* t = NULL;
        t = mxmlFindElement(tnode,xml,"PORT",NULL,NULL,MXML_DESCEND);
        if(t){
            //sscanf(t->child->value.text.string,"%d",&(servers->port));
            sscanf(t->child->value.opaque,"%d",&(servers->port));
        }
...
sys_conf_rtdb_fes_domain* sys_conf_get_rtdb_fes_domain(){
    ...
    cmn_log_log(CMN_LOG_LEVEL_DEBUG,"%s","[sys_conf_get_rtdb_fes_domain](start)");
    ...
    //
    mxml_node_t* p_node = mxmlFindElement(xml,xml,"MODEL","name","rtdb-fes-cli",MXML_DESCEND);
    ...
    cmn_log_log(CMN_LOG_LEVEL_DEBUG,"%s","[sys_conf_get_rtdb_fes_server](start)");
    sys_conf_rtdb_fes_server_list* list = NULL;
    list = sys_conf_get_rtdb_fes_server(node,xml);
    cmn_log_log(CMN_LOG_LEVEL_DEBUG,"%s","[sys_conf_get_rtdb_fes_server](end)");

    cmn_log_log(CMN_LOG_LEVEL_DEBUG,"%s","[sys_conf_match_combine_rtdb_fes_server](start)");
    sys_conf_rtdb_fes_server* final = NULL;
    final = sys_conf_match_combine_rtdb_fes_server(worker.datas,list);
    cmn_log_log(CMN_LOG_LEVEL_DEBUG,"%s","[sys_conf_match_combine_rtdb_fes_server](end)");
    ...
    sys_conf_rtdb_fes_domain* ret = (sys_conf_rtdb_fes_domain*)malloc(sizeof(sys_conf_rtdb_fes_domain));
    ret->first = final;
    ret->log = log;
...
int sys_conf_free_rtdb_fes_domain(sys_conf_rtdb_fes_domain* data){
    cmn_log_log(CMN_LOG_LEVEL_DEBUG,"%s","[sys_conf_free_rtdb_fes_domain](start)");
    ...
    sys_conf_rtdb_fes_server* temp = data->first;
    while(temp){
        if(temp->username){
            free(temp->username);
        }
        ...
        sys_conf_rtdb_fes_server* next = temp->next;
        free(temp);
        temp = next;
    }
    free(data);
    cmn_log_log(CMN_LOG_LEVEL_DEBUG,"%s","[sys_conf_free_rtdb_fes_domain](end)");
    cmn_log_uninit();
    return 0;
}
