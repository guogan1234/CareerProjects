...
int test_changed_multiupload_di(time_t curTime)
{
    int iRet = -1;
    ...
    if(!errOccur)
    {
        iRet = irtdb_fes_changed_multiupload_di(dis,disSize); 
        if(0 == iRet)
            printf("irtdb_fes_changed_multiupload_di ok!\n");
        else
            printf("irtdb_fes_changed_multiupload_di fail!\n");
    }    
    for(i = 0; i < disSize ; i++)
    {
        di = dis[i];
...
int test_changed_multiupload_ai(time_t curTime)
{
    int iRet = -1;
    ...
    for(i = 0; i < aisSize ; i++)
    {
        ...
        ai = irtdb_fes_create_fep_ai(pathBuf,456.1 + i,curTime);
        if(!ai)
        {
            errOccur = 1;
            break;
    ...
    if(!errOccur)
    {
        iRet = irtdb_fes_changed_multiupload_ai(ais,aisSize); 
        if(0 == iRet)
            printf("irtdb_fes_changed_multiupload_ai ok!\n");
        else
            printf("irtdb_fes_changed_multiupload_ai fail!\n");
    }    
    for(i = 0; i < aisSize ; i++)
    {
        ai = ais[i];
...
int test_changed_multiupload_acc(time_t curTime)
{
    int iRet = -1;
    ...
    if(!errOccur)
    {
        iRet = irtdb_fes_changed_multiupload_acc(accs,accSize); 
        if(0 == iRet)
            printf("irtdb_fes_changed_multiupload_acc ok!\n");
        else
            printf("irtdb_fes_changed_multiupload_acc fail!\n");
    }    
    for(i = 0; i < accSize ; i++)
    {
        acc = accs[i];
...
int main()
{
    ...
    test_changed_multiupload_di(curTime);
    test_changed_multiupload_ai(curTime);
    test_changed_multiupload_acc(curTime);
