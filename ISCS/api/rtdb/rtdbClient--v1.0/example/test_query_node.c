...
void test_query_node(rtdb_cli* client)
{
    ...
    record = rtdb_cli_query_node(client,"dlabel_01");
    if(record)
    {
        show_record(record); 
    ...
    record = rtdb_cli_query_node(client,"dlabel_01:slabel_01:proLabel_01:devLabel_01");
    if(record)
    {
       show_record(record);
       rtdb_cli_record_free(record);
...
void test_multi_query_nodes(rtdb_cli* client)
{
    ...
    if(!paths) return ;
    rtdb_cli_list_push_tail(paths,"dlabel_01");
