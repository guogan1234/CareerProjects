...
void irtdb_fes_delete_message_bin(void* pMsgBin);
...
struct irtdb_fes_soe_msg {
    irtdb_fes_header *header;
    // domain_label:station_label:pro_system_label:dev_lavel:point_label
    irtdb_fes_path_label *pathLabel;
    
    int32_t valueLen;
    char *value;
	int32_t scaleLen;
    char *timeScale; // max : 32 bytes
    int64_t timestamp;
};
typedef struct irtdb_fes_soe_msg irtdb_fes_soe_msg;

irtdb_fes_soe_msg* irtdb_fes_create_soe_msg(
	irtdb_fes_cmn_path *path, int8_t dataType, const char *value,
	const char *timeScale, int64_t timestamp);
