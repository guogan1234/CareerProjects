...
{
    CfdbService cfdbservice_object;
    cfdbservice_object.getDataMap(this->m_ndomain_id,NODE_CONFIG,&this->m_qmap_config);
    cfdbservice_object.getDataMap(this->m_ndomain_id,COLOR_CONFIG,&this->m_qmap_color);
...
void ConfigDialog::initNodeCombobox(QMap<int, ConfigData> &map)
{
...
void ConfigDialog::initFontDialog(QFont font)
{
...
void ConfigDialog::setColorValue(AlarmColor &color)
{
...
{
    qDebug() << "slotDoubleClickedHeader" << index;
    setEnableManager(true);
    ConfigData *configdata_temp = NULL;
    configdata_temp = getCurrentNode(index);
    ui->m_qtablewidget_node->setCurrentItem(ui->m_qtablewidget_node->item(index,0));
    if(configdata_temp)
...
{
    qDebug() << "slotClickedItem";
    QString qstr_sendername = sender()->objectName();
    if(ui->m_qtablewidget_node->objectName().operator ==(qstr_sendername.toLatin1()))
    {
        ...
        if(pconfigdata_temp)
        {
            if(1 == column)
            {
                if(pconfigdata_temp->m_qstr_config_name.operator ==(ui->m_qtablewidget_node->item(row,column)->text().toLatin1()))
                    return;
    ...
    else if(ui->m_qtablewidget_field->objectName().operator ==(qstr_sendername.toLatin1()))
    {
...
{
    QString qstr_sendername = sender()->objectName();
    if(ui->m_qpushbutton_node_add->objectName().operator ==(qstr_sendername.toLatin1()))
    {
    ...
    else if (ui->m_qpushbutton_node_sub->objectName().operator ==(qstr_sendername.toLatin1()))
    {
        int nitems_count = 0;
        ...
        while(--nitems_count)
        {
            ...
            {
                if(ui->m_qtablewidget_node->selectedItems().at(nstart)->text().operator ==(iterator_config.value().m_qstr_config_name.toLatin1()))
                {
    ...
    else if (ui->m_qpushbutton_field_add->objectName().operator ==(qstr_sendername.toLatin1()))
    {
    ...
    else if(ui->m_qpushbutton_field_sub->objectName().operator ==(qstr_sendername.toLatin1()))
    {
        int nnode_row = ui->m_qtablewidget_node->selectedItems().first()->row();
        ConfigData *configdata_temp = NULL;
        configdata_temp = getCurrentNode(nnode_row);
        if(!configdata_temp)
            return;
        int nfield_count = ui->m_qtablewidget_field->selectedItems().size() / 2;
        if(0 == nfield_count)
            return;
        nfield_count += 1;
        int nstart = 1;
        while(--nfield_count)
        {
            QMap<int,QString>::iterator iterator_field = configdata_temp->m_qmap_fields.begin();
            while(configdata_temp->m_qmap_fields.end() != iterator_field)
            {
                if(ui->m_qtablewidget_field->selectedItems().at(nstart)->text().operator ==(iterator_field.value().toLatin1()))
                {
                    configdata_temp->m_qmap_fields.remove(iterator_field.key());
                    ui->m_qtablewidget_field->removeRow(ui->m_qtablewidget_field->selectedItems().at(nstart)->row());
                    break;
                }
                ++iterator_field;
            }
    ...
    else if(ui->m_qpushbutton_manager_init->objectName().operator ==(qstr_sendername.toLatin1()))
    {
        this->m_qmap_config_current.operator =(this->m_qmap_config);
        initManagerView();
    }
    else if(ui->m_qpushbutton_manager_save->objectName().operator ==(qstr_sendername.toLatin1()))
    {
        CfdbService cfdbservice_object;
        cfdbservice_object.setDataMap(this->m_ndomain_id,NODE_MANAGER,&this->m_qmap_config_current);
...
void ConfigDialog::slotClickedButtonNode(bool status)
{
    ...
    if(qstr_sendname.operator ==(ui->m_qpushbutton_node_save->objectName().toLatin1()))
    {
        CfdbService cfdbservice_object;
        if(0 == cfdbservice_object.setData(this->m_ndomain_id,NODE_CONFIG,getCurrentNode()))
        {
    ...
    else if(qstr_sendname.operator ==(ui->m_qpushbutton_node_init->objectName().toLatin1()))
    {
        QMap<int,ConfigData>::iterator iterator_config = this->m_qmap_config.begin();
        while(this->m_qmap_config.end() != iterator_config)
        {
...
void ConfigDialog::slotClickedRadioNode(bool status)
{
    ...
    int nvalue = 1;
    if(qstr_sendname.operator ==(ui->m_qradiobutton_un->objectName().toLatin1()))
        nvalue = 0;
    else if(qstr_sendname.operator ==(ui->m_qradiobutton_en->objectName().toLatin1()))
        nvalue = 1;
    else if(qstr_sendname.operator ==(ui->m_qradiobutton_flash->objectName().toLatin1()))
        nvalue = 2;
    ...
    {
        if(qstr_fieldname.operator ==(iterator_fields.value().toLatin1()))
        {
...
{
    QString qstr_sendname = this->sender()->objectName();
    if(qstr_sendname.operator ==(ui->m_qcombobox_node->objectName().toLatin1()))
    {
        ConfigData *pconfigdata_temp = NULL;
        pconfigdata_temp = getCurrentNode();
        if(!pconfigdata_temp)
    ...
    else if(qstr_sendname.operator ==(ui->m_qcombobox_field->objectName().toLatin1()))
    {
        QString qstr_fieldname = ui->m_qcombobox_field->currentText();
        if(qstr_fieldname.isEmpty())
        {
...
void ConfigDialog::slotClickedButtonColor(bool status)
{
    ...
    if(qstr_sendname.operator ==(ui->m_qpushbutton_alarm_text->objectName().toLatin1()))
        palarmcolor_temp->m_qstr_alarm_text = setColorButton(ui->m_qpushbutton_alarm_text).toString();
    else if(qstr_sendname.operator ==(ui->m_qpushbutton_alarm_back->objectName().toLatin1()))
        palarmcolor_temp->m_qstr_alarm_back = setColorButton(ui->m_qpushbutton_alarm_back).toString();
    else if(qstr_sendname.operator ==(ui->m_qpushbutton_alarm_flash_text->objectName().toLatin1()))
        palarmcolor_temp->m_qstr_alarm_flash_text = setColorButton(ui->m_qpushbutton_alarm_flash_text).toString();
    else if(qstr_sendname.operator ==(ui->m_qpushbutton_alarm_flash_back->objectName().toLatin1()))
        palarmcolor_temp->m_qstr_alarm_flash_back = setColorButton(ui->m_qpushbutton_alarm_flash_back).toString();
    else if(qstr_sendname.operator ==(ui->m_qpushbutton_recover_text->objectName().toLatin1()))
        palarmcolor_temp->m_qstr_recover_text = setColorButton(ui->m_qpushbutton_recover_text).toString();
    else if(qstr_sendname.operator ==(ui->m_qpushbutton_recover_back->objectName().toLatin1()))
        palarmcolor_temp->m_qstr_recover_back = setColorButton(ui->m_qpushbutton_recover_back).toString();
    else if(qstr_sendname.operator ==(ui->m_qpushbutton_recover_flash_text->objectName().toLatin1()))
        palarmcolor_temp->m_qstr_recover_flash_text = setColorButton(ui->m_qpushbutton_recover_flash_text).toString();
    else if(qstr_sendname.operator ==(ui->m_qpushbutton_recover_flash_back->objectName().toLatin1()))
...
{
    QString qstr_sendname = this->sender()->objectName();
    if(qstr_sendname.operator ==(ui->m_qpushbutton_color_save->objectName().toLatin1()))
    {
        CfdbService cfdbservice_object;
        if(0 == cfdbservice_object.setData(this->m_ndomain_id,COLOR_CONFIG,getCurrentColor()))
            this->m_qmap_color[getCurrentColor()->m_nalarm_level].operator =(*getCurrentColor());
    }
    else if(qstr_sendname.operator ==(ui->m_qpushbutton_color_init->objectName().toLatin1()))
    {
        getCurrentColor()->operator =(this->m_qmap_color.value(getCurrentColor()->m_nalarm_level));
        setColorValue(*getCurrentColor());
    }
    else if(qstr_sendname.operator ==(ui->m_qpushbutton_color_copy->objectName().toLatin1()))
    {
        ColorCopyDialog *colorcopydialog_window = NULL;
        colorcopydialog_window = new ColorCopyDialog(this->m_qmap_color_current);
        colorcopydialog_window->show();
