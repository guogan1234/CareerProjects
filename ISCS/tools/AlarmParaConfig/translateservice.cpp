...
QString TransLateService::TransLateMapToJson(const QMap<int, QString> fields, const QMap<int, int> policy)
{
...
int TransLateService::TransLateJsonToMap(const QString json, QMap<int, QString> &field, QMap<int, int> &policy)
{
    ...
    {
        field.insert(iterator_json.key().toInt(),TransLateJsonValueToFieldName(iterator_json.value().toString()));
        policy.insert(iterator_json.key().toInt(),TransLateJsonValueToPolicy(iterator_json.value().toString()));
        ++iterator_json;
    }
    return 0;
}

QString TransLateService::TransLateJsonValueToFieldName(QString value)
{
    if(value.isEmpty())
        return "";
    int start = 0;
...
int TransLateService::TransLateJsonValueToPolicy(QString value)
{
    if(value.isEmpty())
        return -1;
